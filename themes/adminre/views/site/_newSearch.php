...
if(count($suppliers)>3)
	$selectText	=	'Found 5 teams matching your requirement';
else
	$selectText	=	'Select team to get connected';
if(count($suppliers)==1)
	$mainText	=	'Here is '.count($suppliers).' team we recommend.';
else
	$mainText	=	'Here are the '.count($suppliers).' teams we recommend.';
if($count==0 && $filterApplied !=0 && $match!=0)
	echo '<div class="col-md-12 col-sm-12 col-xs-12 blank-div text-center" id="blank-div-height">
			<div class="col-md-12 col-sm-12 col-xs-12 blank-div-layout">
				<div class="col-md-12 col-sm-12 col-xs-12 mb30">
					<div class="circle-search">	
						<span class="icon-magnifier white-text fs36" aria-hidden="true"></span>
					</div>
				</div>
				<span class="fs22 font_newlight blue-new">Your search was too narrow! <br>But we have awesome teams on our waitlist that you might like.</span>
				<div class="col-md-12 col-sm-12 col-xs-12 mt20">
					<a class="btn btn-default3 font_newregular fs14 pt5 pb5" href="'.Yii::app()->createUrl('/site/project').'">Help Me Dig Out The Right Team</a>
				</div>
			</div>
		</div>';
else if(count($suppliers)==0 && $filterApplied==0 && $match!=0)	
// For No parameter selected
	echo '<div class="col-md-12 col-sm-12 col-xs-12 blank-div text-center rs-np" id="blank-div-height">
			<div class="col-md-12 col-sm-12 col-xs-12 blank-div-layout rs-np">
				<div class="col-md-12 col-sm-12 col-xs-12 mb30">
					<div class="circle-search">	
						<span class="icon-magnifier white-text fs36" aria-hidden="true"></span>
					</div>
				</div>
				<span class="fs22 font_newlight blue-new">No Filters Applied! <br>But we have awesome teams on our waitlist that you might like.</span>
				<div class="col-md-12 col-sm-12 col-xs-12 mt20">
					<a class="btn btn-default3 font_newregular fs14 pt5 pb5" href="'.Yii::app()->createUrl('/site/project').'">Help Me Dig Out The Right Team</a>
				</div>
			</div>
	...
	if(count($suppliers)>0 && $match==1)
		echo '<h1 class="fs22 font_newlight dark_blue_new display_inline mt15 mb0 pa10 ml10 grey-top-text">Found '.count($suppliers).' teams matching your requirement</h1>';
	elseif(count($suppliers)>0)
		echo '<h1 id="headShow" class="fs22 font_newlight mt15 mb0 pa10 ml10 grey-top-text '.(($totalShow>8)?' hide':'').'">Similar Teams matching your requirement</h1>';
}	
...
foreach($suppliers as $supplier1){
	...
	//$orList		=	$supplier1['option'];
		...
		?>
<div class="newsearch-div  pa20 pt25 pb25 result-show  <?php echo ($class==1)?'bgwhite':'bgwhite'; echo ($totalShow>8)?' hide':'';?>">
	<div class="tm-hover">
		<div class="col-md-1 col-sm-12 col-xs-12 z-ind-1">
			<a href="<?php echo (!empty($supplier->users->display_name))?Yii::app()->createUrl('/'.$supplier->users->display_name):'javascript:void(0);'; ?>" target="_blank" class="setSkills tm-hover">
				<div class="tm-show2">View Profile</div>
				<img alt="Team Member" class="img-circle" width="80" height="80" src="<?php echo (!empty($supplier->image))?$supplier->image.'/convert?w=80&h=80&fit=crop':Yii::app()->theme->baseUrl.'/style/images/avatar.png';?>">
			</a>
		</div>	
		<div class="col-md-8 col-sm-12 col-xs-12 pl60 pr0 rs-np rs-class">
			<div class="pb20 <?php echo !empty($supplier->offers)?'rs-h1':'rs-h1-blank';?>">
				<h2 class="font_newlight nm ellipsis">
					<?php echo (!empty($supplier->users->display_name))?CHtml::link($supplier->users->company_name,array('/'.$supplier->users->display_name),array('class'=>"setSkills search-head-text fs16 font_newregular",'target'=>'_blank')):'<a href="javascript:void(0);" class="search-head-text fs16 font_newregular">'.$supplier->users->company_name.'</a>';?>
				</h2>
				<a href="javascript:void(0);" data-toggle="modal" data-target="#view-project-<?php echo $supplier->id;?>" class="fs18 mt5 mb20 search-blue ellipsis a-width project-view rs-hide">
				<?php 
				$listKey	=	array();
				if(!empty($data['skills'])){
				...
				if($supplier1['matchCount']>0){
					echo '<span class="font_newregular projects">'.$supplier1['matchCount'];
					echo (($supplier1['matchCount']>1)?' projects':' project').'</span>';
					echo (count($listKey)!=0||count($industrytemp)!=0)?' in ':'';
					echo implode(', ',$listKey);
				...
				<a href="javascript:void(0);" data-toggle="modal" data-target="#r-view-project-<?php echo $supplier->id;?>" class="fs18 mt5 mb20 search-blue ellipsis a-width project-view rs-show hide">
				<?php 
				if($supplier1['matchCount']>0){
			...
			<div class="col-md-12 np pb20 ">
				<?php 
				$locationRegions	=	array();
				if(!empty($searchLocation)){
					foreach($searchLocation as $loc){
						if(!empty($loc))
						{
							$locationFilter							=	explode('#',$loc);
							$locationPreferance						=	$locationFilter[0];
							$locationRegions[$locationPreferance][]	=	$locationFilter[1];
						}
					}
				}
				$cityName		=	'NA';
				...
				$otherLocation	=	array();
				if(!empty($locationRegions)){
					foreach($supplier->users->usersOffices as $office){
							foreach($locationRegions as $locationPreferance =>$val){
								if($cityName=='NA' && $locationPreferance=='City' && in_array($office->city->others,$val)){
									$officeLocCount--;
									$cityName	=	$office->city->name.', '.$office->city->countries->name;
									$dept 		=   $office->dep->name;
								}
								elseif($cityName=='NA' && $locationPreferance=='Country' && in_array($office->city->countries->id,$val)){
									$officeLocCount--;
									$cityName	=	$office->city->name.', '.$office->city->countries->name;
									$dept 		=   $office->dep->name;
								}
								elseif($cityName=='NA' && $locationPreferance=='Regions' && in_array($office->city->countries->regions_id,$val)){
									$officeLocCount--;
									$cityName	=	$office->city->name.', '.$office->city->countries->name;
									$dept 		=   $office->dep->name;
								}
								elseif($cityName=='NA' && $locationPreferance=='Zone' && in_array($office->city->countries->price_zone_id,$val)){
									$officeLocCount--;
									$cityName	=	$office->city->name.', '.$office->city->countries->name;
									$dept 		=   $office->dep->name;
								}
				...
				if($cityName == 'NA'){
					...
					foreach($supplier->users->usersOffices as $office){
						if(($office->dep_id == 3)&&($cityName=='NA')){
							...
							$otherLocation[$office->dep->name] = isset($otherLocation[$office->dep->name])?$otherLocation[$office->dep->name]:array();
							array_push($otherLocation[$office->dep->name], $office->city->name.' , '.$office->city->countries->name);
					...
					$otherLocation=array();
					foreach($supplier->users->usersOffices as $office){
						$value = $office->city->name.', '.$office->city->countries->name.' - '.$office->dep->name;
						if($value != ($cityName.' - '.$dept)){
							$otherLocation[$office->dep->name] = isset($otherLocation[$office->dep->name])?$otherLocation[$office->dep->name]:array();
							array_push($otherLocation[$office->dep->name], $office->city->name.' , '.$office->city->countries->name);
							//$otherLocation[]	= 	$value;	
						}
				...
				?>
						<?php if($cityName != 'NA'){?>
							<span class="icon-pointer darkgrey_iconnew fs14" aria-hidden="true"></span> 
							<div class="display_inline posr <?php if(!empty($locationRegions)) echo 'loc-bg'; ?>">
								<p class="dis_inline darkgrey_new ml2 font_newlight"><?php echo $dept; ?></p> 
								- 
								<p class="dis_inline darkgrey_new ml2 font_newregular"><?php echo $cityName; ?></p>				
							</div>
						<?php //CVarDumper::dump($otherLocation,10,1);die(); 
						}
						foreach($otherLocation as $department=>$locations){ 
						?>
							<div class="display_inline posr">
								<span class="fs4">&nbsp;&nbsp;<i class="fa fa-circle pos-absl"></i></span>
								<p class="dis_inline darkgrey_new ml2 font_newlight"><?php echo $department; ?></p> 
								- 
							<?php
							$count = 1;
							foreach($locations as $location){ ?>
									<p class="dis_inline darkgrey_new ml2 font_newregular"><?php echo $location; ?></p>
									<?php if(count($locations) != $count){ ?>
									<span class="fs4">&nbsp;&nbsp;<i class="fa fa-circle"></i></span>							
							<?php 	}
							$count++;
							}
							?>
				...
				<!-- <div class="more-outr rs-hide">
					...
					<div class="more-location hide pa15">
						...
						<ul class="pl10 mt5">
							<?php //foreach($otherLocation as $department=>$locations){ 
								//foreach($locations as $location){?>
									<li><?php //echo $department.'-'.$location; ?></li>
							<?php //} }?>
						</ul>
			...
			<div class="col-md-12 col-sm-12 col-xs-12 np bt bb ">
				<div class="row pt15 rs-class ">
					
					<div class="col-md-4 col-sm-12 col-xs-12 fs13 ellipsis rs-pa10">
						<span class="icon-users darkgrey_iconnew fs14" aria-hidden="true"></span>
						<p class="ellipsis dis_inline darkgrey_new ml2"><?php echo $supplier->number_of_employee; ?> Engineers </p>   
					</div>
					<?php
					...
					<div class="col-md-4  col-sm-12 col-xs-12 fs13 font_newregular rs-pa10 rs-star rating-pullout">
						...
						<span class="rating-rollover ">
							...
							<img class="toparrow" src="<?php echo Yii::app()->theme->baseUrl."/style/images/profile-rollover-right.png"; ?>" />
						</span>
					</div>
					<?php } ?>
					<div class="col-md-4 col-sm-12 col-xs-12  fs13 trusted-color rs-pa10"><span class="icon-check fs14" aria-hidden="true"></span> <span class="ml2">Trusted</span>  </div>
				</div>  
				<div class="row pb15 rs-class <?php echo ($avgRating > 0)?'':'mt5'; ?>">
					<div class="col-md-4 col-sm-12 col-xs-12  fs13 ellipsis rs-pa10">
						<span class="icon-tag darkgrey_iconnew fs14" aria-hidden="true"></span> 
						<p class="ellipsis dis_inline darkgrey_new ml2">
						<?php 
							$rate 	   = '';
					...
					<?php if($ratCount>0){ ?>
					<div class="col-md-4 col-sm-12 col-xs-12 fs13 darkgrey_new rs-pa10">
						<span><?php echo ($ratCount>0)?$ratCount:'No'; ?> Reviews</span>
					</div>
					<?php } ?>
			...
			<div class="col-md-12 col-sm-12 col-xs-12 np ">
				<div class="col-md-12 col-sm-12 col-xs-12 mt25 np rs-show rs-mt10 rs-pb15">
					<div class="col-md-12 col-sm-12 col-xs-12 np mt10">
						<?php foreach($skills as $key=>$val){?>
								<span class="btn-sm-new mb5"><?php echo $key;?></span>
								<!--<span class="btn-sm-newdark mr5 mb5"><?php //echo $val; ?></span>-->
						<?php	}
						...
						<div class="skills-outr rs-hide">
							...
							<!--<div class="skills-opn2 btn-sm hide pa10 ">
				...
				<p class="pt10 col-md-12 np"><span class="fs14 darkgrey_new">Clients: </span> <span class="gray-light"><span ><?php echo implode('&nbsp  •  &nbsp',array_slice($listOfCompany, 0, 5)); ?> <?php if($otherCount>0){echo (!empty($supplier->users->display_name))?CHtml::link('&nbsp ('.$otherCount.' more)',array('/'.$supplier->users->display_name),array('class'=>"setSkills orange-new fs10 pt5 w80 rs-hide",'target'=>'_blank')):'<a  href="javascript:void(0);" class="orange-new fs10 pt5 w80"> ('.$otherCount.' more)</a>';} ?> </span></span></p>
			</div>
		</div>
	</div>
	<div class="col-md-3 col-sm-12 col-xs-12 pt10 guest rs-mt15">
			
		<button type="button" id="not-selected-<?php echo $supplier->id;?>" value="<?php echo $supplier->id;?>" class="btn btn-default3 dropdown-toggle font_newregular fs14 col-md-12 col-sm-12 col-xs-12 hide-result pb10 pt10">Select Team</button>
		<button type="button" id="selected-<?php echo $supplier->id;?>" name="supplierList[]" value="<?php echo $supplier->id;?>" class="btn btn btn-orange pb5 dropdown-toggle font_newregular col-md-12 col-sm-12 col-xs-12 fs14  pull-right show-result pb10 pt10 hide"><i class="fa fa-check"></i> Selected</button>
	
		<div class="col-md-12 col-sm-12 col-xs-12 mt25 np rs-hide">
			<div class="col-md-12 col-sm-12 col-xs-12 np mt10">
				<?php foreach($skills as $key=>$val){?>
					<div class="pull-left" >
						<span class="btn-sm-new mb5"><?php echo $key;?></span>
				...
				<div class="skills-outr">
					...
					<div class="more-skills1 hide pa10">
						<?php echo $countSk;?> more
						<div class="mt10">
							<?php foreach($otherSkills as $key=>$val){?>
							<div class="pull-left">
...
<?php
echo ($totalShow==8 && count($suppliers)>8)?'<div class="col-md-12 col-xs-12 text-center light-g"><a href="javascript:void(0);" class="btn status-btn dropdown-toggle font_newregular fs12 mr10 mt15 mb15 pl50 pr50 pt10 pb10" id="see-more">View More</a></div>':'';
if($class==0){$class=1;}else{$class=0;}
?>
...
<div class="modal fade modal-lazy" id="view-project-<?php echo $supplier->id;?>" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
	<div class="modal-dialog">
		<div class="modal-content col-md-12 col-sm-12 col-xs-12 np">
			...
			<div class="modal-body col-md-12 col-sm-12 col-xs-12 np new-modal-bg pb15">
				<div class="col-md-12 col-sm-12 col-xs-12 np">
										...
										<div class="col-md-12 col-sm-12 col-xs-12 np pl20 pr20 mt30 mb0" id="reviews-<?php echo $supRef->id; ?>">		
											...
											<div class="col-md-8 col-sm-12 col-xs-12 pa10 bl pl30">
												...
												<div class="col-md-12 col-sm-12 col-xs-12 mt10 np">
													<h3 class="mt0 display_inline mr5 fs13 blue-new font_newregular mb15"><?php echo $question->reviewQuestions->title;?></h3>
