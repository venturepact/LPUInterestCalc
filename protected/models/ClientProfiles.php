...
class ClientProfiles extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('users_id, status, manager_id', 'numerical', 'integerOnly'=>true),
			...
			array('id, users_id, first_name, last_name, company_name, company_link, skype_id, email, phone_number, address1, team_size, category, foundation_year, image, description, add_date, status, manager_id', 'safe', 'on'=>'search'),
	...
	public function profileSearch()
	{
		...
		$criteria->compare('users.first_name', $this->user_firstname,true);
		$criteria->compare('users.username', $this->user_email,true);
		
		$criteria->with = array(
		                	'users'=>array('select'=>'users.company_name, users.first_name, users.username'),
		                );
	...
	/**
	*
	* Changes lead owners for clients, and all its related projects
	* @param integer $manager_id Identifier of the account manager
	* @param object $client Object of client profile
	* @return boolean $result Success or Failure
	*
	**/

	public function updateLeadOwner($manager_id, $client) {

		// get the manager
		$manager = Users::model()->findByAttributes(array('id'=>$manager_id, 'role_id'=>'1'));
		if(empty($manager)) {
			return false;
		}

		// begin transaction before updating data
		...
		// for each of this client's projects -> change manager
		foreach ($client->clientProjects as $cProject) {
			foreach($cProject->suppliersProjects as $sProject) {
				$leadOwner = $manager_id;

				$managerExists = ChatRoomHasUsers::model()->findByAttributes(array('users_id'=>$leadOwner, 'chat_room_id'=>$sProject->chat_room_id));
				if(!$managerExists) {
					$managerChatUser = new ChatRoomHasUsers;
					$managerChatUser->chat_room_id = $sProject->chat_room_id;
			        $managerChatUser->users_id = $leadOwner;
			        $managerChatUser->users_type = 1;
			        $managerChatUser->save();
				}

				// find all account managers for this chat
				$criteria = new CDbCriteria;
				$criteria->condition = "chat_room_id = $sProject->chat_room_id";
				$criteria->addInCondition('users_type', array('1', '2'));
				$updateLeadUsers = ChatRoomHasUsers::model()->updateAll(array('users_type'=>1), $criteria);

				unset($criteria);
				$criteria = new CDbCriteria;
				$criteria->condition = "chat_room_id = $sProject->chat_room_id AND users_id=" . $leadOwner;
				$criteria->addInCondition('users_type', array('1', '2'));
				$leadChatUsers = ChatRoomHasUsers::model()->findAll($criteria);
				$updateLeadUsers = ChatRoomHasUsers::model()->updateAll(array('users_type'=>2), $criteria);

			} // end foreach - supplier projects
		} // end foreach - client projects
