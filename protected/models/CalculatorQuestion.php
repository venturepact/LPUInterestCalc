...
class CalculatorQuestion extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('parent, category_id, multi, is_deleted, is_cal_lpu', 'numerical', 'integerOnly'=>true),
			array('question', 'length', 'max'=>500),
			array('description, created, modified', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, parent, category_id, question, description, multi, created, modified, is_deleted, is_cal_lpu', 'safe', 'on'=>'search'),
		);
	}
